...
审计基线提交：`d5d70c5a7490273b31256c1a774530a2e8512b1e`

Simulation Contract：`0.1.3`

审计结论：**M1 未通过，状态为 `not_passed_gaps`**
...
但是，冻结的 M1 baseline 明确要求 FIFO/SPT/EDD/CR 共享统一动作链、跨策略实体索引共同随机数证据，以及 `simulate(theta, scenario, seed)` 接口。当前只有 FIFO，后两项也没有完整证据或接口。根据“全部满足”规则，M1 不能标记为 passed。审计没有修改原验收标准。
...
| M1-E06 | 必需 | FIFO、SPT、EDD、CR 使用相同候选生成、可行性检查和动作提交器 | 仅有 `policies/base.py`、`policies/fifo.py` | **GAP** | SPT、EDD、CR 未实现，无法证明四策略共用动作链 |
| M1-E07 | 必需 | 同场景/代码/契约/种子严格复现；跨策略使用实体索引共同随机场景 | `random_streams.py` 与 determinism 测试 | **GAP** | 单策略及实体索引流已验证；因仅有 FIFO，跨策略 Common Random Numbers 尚无法验证 |
...
| M1-E10 | 必需 | `simulate(theta, scenario, seed)` 存在，优化命令在 M1 通过前拒绝运行 | `optimizer_enabled=false`；代码搜索无该接口 | **GAP** | 禁用门有效；规定接口不存在 |
...
- SPT、EDD、CR 与统一 `simulate(...)` 接口不是“后续可选项”，因为原 M1 baseline 已将其列为必需条件；本阶段因用户禁止实现而保留 GAP。
...
| FIFO | Contract §3 / M1-E06 | `policies/base.py`、`fifo.py` | MC02 与 policy tests | PASS（仅 FIFO） |
...
| Random streams | Contract §11 | `random_streams.py` | failure/repair/PM call-order tests | PARTIAL |
| Provenance | Contract §10/11 | `provenance.py`、Scenario serialization | MC04/MC06/MC07/MC08 与 completeness tests | PASS（手工 Scenario） |
| Metrics | Contract §10 | `engine.py`、`evaluation/audit.py` | closure audit tests | PARTIAL；见 Metric Contract |
...
1. **Contract 有定义、代码未实现**：Transport、sampling/rework、随机 processing 及 load/unload/cascading、部分正式指标、SPT/EDD/CR、统一 `simulate(...)`。
...
GAP：release、processing、transport、sampling/rework 与 batch processing 的正式分布流尚未接入；仅有 FIFO，无法用多策略实验证明跨策略 Common Random Numbers。因此 M1-E07 不能通过。

## 9. Provenance audit

MC04、MC07、MC08 的结果保存：Contract version、dataset identity/version、git commit、seed、policy、termination mode、horizon、完整 Scenario。Scenario 中可恢复 machine、lot、operation、Batch、CQT、Dedication、Failure 和 PM 配置，以及随机流 scheme/version。
...
所有 fixture、结果与当前配置统一声明 `0.1.3`；MC01～MC08 在当前版本下全部重跑。0.1.1～0.1.3 是对 Dedication、Failure、PM 的向后兼容 patch：没有改变 MC01～MC05 的既有语义。审计中修复了一个实现偏差：Mean Cycle Time 现在按 Data Contract 排除缺少真实 release history 的 initial WIP cohort。该修复使代码符合既有 Contract，不改变业务语义，因此 Contract 仍为 `0.1.3`。
...
**VERIFIED WITHIN IMPLEMENTED SCOPE**：对正确构造的 Scenario，MC01～MC08 与新增审计不变量在 Contract `0.1.3` 下通过。该结论不覆盖尚未实现的 transport、sampling/rework、正式 processing distributions 和完整指标。
...
**M1 = `not_passed_gaps`**。阻断项为 M1-E06、M1-E07、M1-E10。审计没有为了通过而修改验收标准。

## 14. Next gate

下一阶段唯一建议是 **M1 Exit-Criteria Gap Closure + SMT2020 Data Integration Gate**，按顺序完成：

1. 在不启用优化器的前提下补齐原 M1 明列的 SPT/EDD/CR 共用动作链和统一 `simulate(theta, scenario, seed)` 接口；
2. 用至少两个策略验证实体索引 Common Random Numbers；
3. 建立正式 raw-field → internal Scenario loader 与 manifest/hash provenance；
4. 对每类字段做小切片的 loader golden test 和数据质量审计；
5. 重新执行 M1 Closure Audit 和 Data Integration Gate。

在两项 Gate 通过前：不允许正式 HVLM/LVHM 实验，不允许 CMA-ES，`optimizer_enabled` 继续为 `false`。
