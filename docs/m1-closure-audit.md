...
> 后续状态（2026-09-25）：Simulation Contract `0.1.5` 与 Loader Contract `0.1.4` 增加 Sampling profile。None/100%/随机百分比、operation-entry skip、initial WIP、CRN、provenance 与独立审计形成证据链；raw sampled CQT targets 4/18 全为 p100，stochastic endpoint 为 0，sampling blocker 已关闭。全部 sampled 工序的 1 分钟 load/unload 未进入诊断 slice duration，故 load/unload blocker保留；Data Integration Gate 仍为 5 类 blocker / `not_passed_gaps`，`optimizer_enabled=false`。
